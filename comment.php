...
<li id="comment-<?php comment_ID(); ?>" <?php comment_class( empty( $args['has_children'] ) ? '' : 'parent' ); ?>>
	<article id="div-comment-<?php comment_ID(); ?>" class="<?php echo $comment_body_class; ?>">
		<header class="comment-meta">
			<div class="comment-author vcard">
				<?php if ( 'trackback' == $comment->comment_type || 'pingback' == $comment->comment_type ) { ?>
					<div class="avatar"><i class="focus-icon-link"></i></div>
				<?php } else { ?>
			...
			<p class="comment-awaiting-moderation"><?php _e( 'Your comment is awaiting moderation.', 'focus' ); ?></p>
			<?php endif; ?>
		</header><!-- .comment-meta -->
		...
		<footer class="comment-metadata">
			...
			<?php edit_comment_link( __( 'Edit', 'focus' ), '<span class="edit-comment">', '</span>' ); ?>
